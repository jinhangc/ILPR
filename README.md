...
## Abstract:
We study a contextual dynamic pricing problem under a semiparametric demand model, where the purchase probability takes the form $1 - F(p - m(\boldsymbol x))$ with mean utility $m(\boldsymbol x)$ given the product feature $\boldsymbol x$ and the market noise distribution $F$. Existing approaches for this model either suffer from suboptimal regret rates or require strong structural assumptions that limit flexibility. 
